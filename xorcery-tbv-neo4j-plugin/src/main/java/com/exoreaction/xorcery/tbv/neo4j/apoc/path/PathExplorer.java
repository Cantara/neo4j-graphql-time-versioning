...
public class PathExplorer {
    ...
    @Procedure("tbv.path.expand")
    @Description("tbv.path.expand(startNode <id>|Node|list, 'TYPE|TYPE_OUT>|<TYPE_IN', '+YesLabel|-NoLabel', minLevel, maxLevel ) yield path - expand from start node following the given relationships from min to max-level adhering to the label filters")
    ...
    private Stream<Path> expandConfigPrivate(@Name("start") Object start, @Name("config") Map<String, Object> config) throws Exception {
        ...
        Stream<Path> results = explorePathPrivate(nodes, relationshipFilter, labelFilter, minLevel, maxLevel, bfs, getUniqueness(uniqueness), filterStartNode, limit, nodeFilter, sequence, beginSequenceAtStart, snapshot);
    ...
    public static Traverser traverse(TraversalDescription traversalDescription,
        ...
        // add time-based-versioning evaluator which will only include paths where end-node is an INSTANCE or an EMBEDDED
        td = td.evaluator(new TBVLabelEvaluator());
        ...
        if (sequence != null && !sequence.trim().isEmpty()) {
            ...
            td = td.expand(new RelationshipSequenceExpander(snapshot, relSequenceList, beginSequenceAtStart));
            td = td.evaluator(new LabelSequenceEvaluator(labelSequenceList, filterStartNode, beginSequenceAtStart, (int) minLevel));
        } else {
            String effectivePathFilter = ofNullable(pathFilter).map(String::trim).orElse("");
            td = td.expand(new RelationshipSequenceExpander(snapshot, effectivePathFilter, beginSequenceAtStart));
        ...
        if (minLevel != -1) td = td.evaluator(TBVEvaluators.fromDepth((int) minLevel));
        if (maxLevel != -1) td = td.evaluator(TBVEvaluators.toDepth((int) maxLevel));
