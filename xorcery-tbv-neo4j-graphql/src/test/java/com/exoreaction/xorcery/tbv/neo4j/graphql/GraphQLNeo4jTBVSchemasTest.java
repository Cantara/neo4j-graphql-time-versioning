...
public class GraphQLNeo4jTBVSchemasTest {
    ...
    @Test
    public void thatOrganizationAndEmployeeGraphQLQueryCanBeTranslatedToCorrectCypher() {
        File schemaFile = new File("src/test/resources/schema-samples/organization_and_employee.graphql");
        TypeDefinitionRegistry definitionRegistry = parseSchemaFile(schemaFile);
        GraphQLSchema schema = GraphQLNeo4jTBVSchemas.schemaOf(GraphQLNeo4jTBVLanguage.transformRegistry(definitionRegistry, true)).transform(builder -> {
            builder.additionalDirectives(Set.of(
                    TBVDirectives.DOMAIN,
                    TBVDirectives.LINK,
                    TBVDirectives.REVERSE_LINK
